...
chrome_path=r"C:\Users\Sholpan\Desktop\progs_and_others\stud matirials\WinPython-64bit-3.4.3.7\python-3.4.3.amd64\Scripts\chromedriver.exe"
        ...
        for comm in comms:     
            for k in range(1, 7):
                if k==1:
                    ...
                    dislikes = comm.find_element_by_xpath('.//div[@class="answer__caption cf"]/span[@class="answer__vote"]/div[@class="answer__down"]/span[@class="answer__value"]').text
                    worksheet.write(cnt, k, dislikes)
                elif k==5:
                    dates = comm.find_element_by_xpath('.//div[@class="answer__caption cf"]/span[@class="answer__time"]').text
                    worksheet.write(cnt, k, dates)
                elif k==6:
